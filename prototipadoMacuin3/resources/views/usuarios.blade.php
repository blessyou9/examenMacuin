...
@section('contenido')
    ...
    <head>
        ...
        <script>
            ...
            function validarUser(event) {
                // Cancelar la acción predeterminada del clic
                event.preventDefault();

                // Mostrar mensaje de confirmación al usuario
                notie.confirm({
                    text: '¿Estás seguro que quieres hacer un nuevo perfil de usuario?<br>',
                    cancelCallback: function() {
                        notie.alert({
                            type: 3,
                            text: 'Creación de nuevo usuario cancelado :('
                        })
                    },
    ...
    <body>
        ...
        <div class="bg-white p-8 rounded-md w-full">
            <div class=" flex items-center justify-between pb-6">
                <div>
                    <h2 class="text-gray-600 font-semibold">CRUD</h2>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="lg:ml-40 ml-10 space-x-8">
                        <a href="/insertarusuario"
                            class="bg-indigo-600 px-4 py-2 rounded-md text-white font-semibold tracking-wide cursor-pointer"
                            onclick="validarUser(event)">New user</a>
                    </div>
                </div>
                            ...
                            <tbody>
                                @foreach ($consultaConsultas as $consulta)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('editarusuario', $consulta->id) }}"
                                                onclick="event.preventDefault(); if(confirm('¿Estás seguro que quieres editar este usuario?')) { window.location.href = '{{ route('editarusuario', $consulta->id) }}'; }"
                                                class="edit" title="Edit" data-toggle="tooltip"><i
                                                    class="material-icons">&#xE254;</i></a>

                                            <a href="{{ route('Eliminar', $consulta->id) }}"
                                            ...
                                            <form id="delete-form-{{ $consulta->id }}"
                                                action="{{ route('Eliminar', $consulta->id) }}" method="POST"
                                                style="display: none;">
                                                {{ csrf_field() }}
                                                {{ method_field('DELETE') }}
                                            </form>
